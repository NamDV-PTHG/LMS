...
import {
  ChevronRight, Users, BookOpen, BarChart2, CheckCircle2, X, ChevronLeft,
  ChevronDown, Clock, Award, FileText, Video, ClipboardList,
} from 'lucide-react';
...
interface ChildStat {
  ...
  memberCount: number; enrolled: number; completed: number; completionRate: number; avgProgress: number;
}
interface Employee {
...
export default function MyDepartmentPage() {
  ...
  const loadOrg = async (orgId: string, orgName: string, isNewBreadcrumb = true) => {
    ...
    try {
      ...
      if (isNewBreadcrumb) setBreadcrumb((prev) => [...prev, { id: orgId, name: orgName }]);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="p-6 max-w-5xl mx-auto">
        ...
        {managedOrgs.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {managedOrgs.map((org) => (
              <button key={org.id} onClick={() => loadOrg(org.id, org.name)}
                ...
                <p className="mt-1 flex items-center gap-1 text-[11px] text-subtle">Xem báo cáo <ChevronRight size={12} /></p>
              </button>
            ))}
          </div>
        )}
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-center gap-1.5 mb-4 flex-wrap">
        ...
        <button onClick={() => { setBreadcrumb([]); setCurrentOrgId(null); setChildren(null); setEmployees(null); }}
        ...
        {breadcrumb.map((crumb, i) => (
          <span key={crumb.id} className="flex items-center gap-1.5">
            ...
            {i < breadcrumb.length - 1 ? (
              <button onClick={() => navigateBreadcrumb(i)} className="text-[12px] text-subtle hover:text-primary transition-colors">{crumb.name}</button>
            ) : (
      ...
      {loading && <div className="text-center py-16 text-subtle text-[13px]">Đang tải dữ liệu...</div>}
      ...
      {!loading && view === 'children' && children !== null && (
        children.length === 0 ? (
          <div className="text-center py-12 text-subtle">
            ...
            <button onClick={loadEmployees} className="mt-3 text-[12px] text-primary underline">Xem tất cả nhân viên →</button>
          </div>
        ) : (
            ...
            <table className="w-full text-[12px]">
              <thead>
                <tr className="border-b border-default bg-muted/30">
                  ...
                  <th className="text-right px-4 py-3 font-semibold text-subtle">NV</th>
                  <th className="text-right px-4 py-3 font-semibold text-subtle">Đăng ký KH</th>
                  <th className="text-right px-4 py-3 font-semibold text-subtle">Hoàn thành KH</th>
                  <th className="px-4 py-3 font-semibold text-subtle min-w-[160px]">Tiến độ TB</th>
              ...
              <tbody>
                {children.map((child) => (
                  <tr key={child.orgId} className="border-b border-default/50 hover:bg-muted/20 transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          {child.leader && <p className="text-[10px] text-subtle">Trưởng: {child.leader.fullName}</p>}
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-right text-content">{child.memberCount}</td>
                    <td className="px-4 py-3 text-right text-content">{child.enrolled}</td>
                    <td className="px-4 py-3 text-right">
                      <span className={`font-medium ${child.completionRate >= 80 ? 'text-green-600' : child.completionRate >= 50 ? 'text-amber-600' : 'text-red-500'}`}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <div className="flex-1 h-1.5 bg-gray-200 rounded-full overflow-hidden">
                          <div className="h-full bg-blue-500 rounded-full" style={{ width: `${child.avgProgress}%` }} />
                        </div>
                    ...
                    <td className="px-4 py-3 text-right">
                      {child.hasChildren ? (
                        <button onClick={() => loadOrg(child.orgId, child.orgName)} className="flex items-center gap-1 text-primary hover:underline ml-auto">
                        ...
                        <button onClick={async () => {
                          ...
                          setLoading(true); setView('employees');
          ...
          <table className="w-full text-[12px]">
            <thead>
              <tr className="border-b border-default bg-muted/30">
                ...
                <th className="text-right px-4 py-3 font-semibold text-subtle">Khóa học</th>
                <th className="text-right px-4 py-3 font-semibold text-subtle">KH hoàn thành</th>
                <th className="px-4 py-3 font-semibold text-subtle min-w-[180px]">Tiến độ bài học</th>
              </tr>
            </thead>
            <tbody>
              {employees.length === 0 && (
                <tr><td colSpan={5} className="px-4 py-10 text-center text-subtle">Không có nhân viên.</td></tr>
              )}
              {employees.map((emp) => (
                <tr key={emp.id} onClick={() => loadEmployeeDetail(emp.id)}
                  className="border-b border-default/50 hover:bg-muted/20 transition-colors cursor-pointer">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <div className="flex-1 h-1.5 bg-gray-200 rounded-full overflow-hidden">
                        <div className={`h-full rounded-full ${emp.lessonProgress >= 80 ? 'bg-green-500' : emp.lessonProgress >= 50 ? 'bg-amber-500' : 'bg-blue-400'}`}
                          style={{ width: `${emp.lessonProgress}%` }} />
                      </div>
                      <span className="text-[11px] text-content font-medium whitespace-nowrap">
          ...
          <div className="relative w-full max-w-xl bg-white h-full shadow-2xl flex flex-col overflow-hidden">
              ...
              return (
                <div className="grid grid-cols-3 gap-0 border-b border-gray-100">
                  <div className="px-4 py-3 text-center border-r border-gray-100">
                    <p className="text-[18px] font-semibold text-content">{completedCourses}/{totalCourses}</p>
                    <p className="text-[10px] text-subtle mt-0.5">Khóa học hoàn thành</p>
                  </div>
                  <div className="px-4 py-3 text-center border-r border-gray-100">
                    <p className="text-[18px] font-semibold text-content">{completedLessons}/{totalLessons}</p>
                    <p className="text-[10px] text-subtle mt-0.5">Bài học hoàn thành</p>
                  </div>
                  <div className="px-4 py-3 text-center">
                    <p className={`text-[18px] font-semibold ${lessonPct >= 80 ? 'text-green-600' : lessonPct >= 50 ? 'text-amber-600' : 'text-blue-600'}`}>
                      {lessonPct}%
                    </p>
                    <p className="text-[10px] text-subtle mt-0.5">Tiến độ tổng</p>
                  </div>
                </div>
              );
            })()}

            {/* Course list with lesson breakdown */}
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              ...
              {selectedEmployee.courses.map((c) => {
                ...
                const isExpanded = expandedSections[courseKey] ?? false;
                return (
                  <div key={c.courseId} className="border border-gray-100 rounded-xl overflow-hidden">
                    {/* Course header */}
                    <button
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <p className="text-[13px] font-semibold text-gray-800 truncate">{c.courseTitle}</p>
                          {c.completedAt
                            ? <span className="shrink-0 text-[10px] font-medium text-green-600 bg-green-50 px-1.5 py-0.5 rounded-full flex items-center gap-1"><CheckCircle2 size={9} /> Hoàn thành</span>
                            : c.completedLessons > 0
                            ? <span className="shrink-0 text-[10px] font-medium text-amber-600 bg-amber-50 px-1.5 py-0.5 rounded-full">Đang học</span>
                            : <span className="shrink-0 text-[10px] font-medium text-gray-400 bg-gray-50 px-1.5 py-0.5 rounded-full">Chưa bắt đầu</span>
                          }
                        </div>
                        {/* Progress bar */}
                        <div className="flex items-center gap-2 mb-1.5">
                          <div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                            <div className={`h-full rounded-full transition-all ${c.completedAt ? 'bg-green-500' : 'bg-blue-500'}`}
                        ...
                        <div className="flex flex-wrap gap-3 text-[10px] text-gray-400">
                          ...
                          {c.certificate && (
                            <span className="flex items-center gap-0.5 text-blue-600 font-medium">
                              <Award size={9} /> Chứng chỉ #{c.certificate.code}
                            </span>
                          )}
                        </div>
                      </div>
                      <ChevronDown size={14} className={`text-gray-400 transition-transform shrink-0 mt-0.5 ${isExpanded ? 'rotate-180' : ''}`} />
                    </button>

                    {/* Section/Lesson breakdown — expandable */}
                    {isExpanded && (
                      <div className="border-t border-gray-100 divide-y divide-gray-50">
                        {c.sections.length === 0 && (
                          <p className="px-4 py-3 text-[12px] text-gray-400">Chưa có bài học nào.</p>
                        )}
                        {c.sections.map((sec) => {
                          const secDone = sec.lessons.filter(l => l.status === 'completed').length;
                          const secTotal = sec.lessons.length;
                          const secKey = `${courseKey}-${sec.id}`;
                          ...
                          return (
                            <div key={sec.id}>
                              ...
                              <button
                                ...
                                className="w-full text-left flex items-center gap-2 px-4 py-2.5 bg-gray-50 hover:bg-gray-100 transition-colors">
                                <BarChart2 size={12} className="text-gray-400 flex-shrink-0" />
                                <span className="flex-1 text-[12px] font-medium text-gray-700 truncate">{sec.title}</span>
                                <span className={`text-[10px] font-medium shrink-0 ${secDone === secTotal && secTotal > 0 ? 'text-green-600' : 'text-gray-400'}`}>
                                  {secDone}/{secTotal} bài
                                </span>
                                <ChevronDown size={12} className={`text-gray-300 transition-transform shrink-0 ${secExpanded ? 'rotate-180' : ''}`} />
                              </button>
                              {/* Lesson list */}
                              {secExpanded && (
                                <div className="divide-y divide-gray-50">
                                  {sec.lessons.map((les) => (
                                    <div key={les.id} className="flex items-start gap-3 px-4 py-2.5 bg-white">
                                      <div className="mt-0.5 flex-shrink-0">
                                        <ContentTypeIcon type={les.contentType} />
                                      </div>
                                      <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-1.5 flex-wrap">
                                          <p className="text-[12px] text-gray-700 truncate flex-1">{les.title}</p>
                                          {les.isRequired && (
                                            <span className="text-[9px] font-semibold text-orange-600 bg-orange-50 px-1 py-0.5 rounded shrink-0">Bắt buộc</span>
                                          )}
                                        ...
                                        {les.status === 'in_progress' && (
                                          <div className="flex items-center gap-1.5 mt-1">
                                            <div className="flex-1 h-1 bg-gray-100 rounded-full overflow-hidden">
                                              <div className="h-full bg-amber-400 rounded-full" style={{ width: `${les.progressPct}%` }} />
                                            </div>
                                            <span className="text-[10px] text-gray-400">{les.progressPct}%</span>
                                          </div>
                                        )}
                                        {/* Quiz result */}
                                        {les.quiz && (
